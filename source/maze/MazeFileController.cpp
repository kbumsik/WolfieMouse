...
void Maze::writeMazeToFile(void *pFile, bool isShowMouse)
{
    char buf;
...
void Maze::printCell(int row, int col, bool isShowMouse, void *pFile)
{
    ...
    if (getCell(row, col).isMouse && isShowMouse) {
        ...
        fputc(
                (getCell(row, col).distance == MAZE_UNREACHED) ?
