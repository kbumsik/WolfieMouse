...
static TIM_TypeDef *_get_device(kb_timer_device_t dev);
/* Get a HAL library handler from the object */
static inline TIM_HandleTypeDef * _get_handler (kb_timer_t *obj);
static uint32_t _get_bus_freq(kb_timer_device_t timer);
static void _enable_device_clk (kb_timer_device_t timer);


/******************************************************************************
 * Public API Functions
 ******************************************************************************/
int kb_timer_ch_pin(kb_timer_device_t dev, kb_timer_ch_t channel, kb_gpio_port_t port, kb_gpio_pin_t pin, kb_gpio_pull_t pull)
{
    uint32_t alternate;
...
int kb_pwm_init(kb_timer_t *obj, kb_pwm_init_t *setting)
{
    ...
    obj->ext_handler = (kb_extlib_handle_t)malloc(sizeof(TIM_HandleTypeDef));
...
int kb_pwm_percent(kb_timer_t *obj, kb_timer_ch_t channel, uint8_t duty_cycle_percent)
{
    return kb_pwm_permyriad(obj, channel, duty_cycle_percent * 100);
}

// 1 permyriad = 0.01%. 10,000 permyriad = 1%. max 10,000 permyriad
int kb_pwm_permyriad(kb_timer_t *obj, kb_timer_ch_t channel, uint16_t duty_cycle_permyriad)
{
    // select handler
    TIM_HandleTypeDef* handler = _get_handler(obj);
    if (NULL == handler) {
...
int kb_pwm_start(kb_timer_t *obj, kb_timer_ch_t channel)
{
...
int kb_pwm_stop(kb_timer_t *obj, kb_timer_ch_t channel)
{
    // get handler and enable timer
    TIM_HandleTypeDef* handler = _get_handler(obj);
    if (NULL == handler) {
...
static void _enable_device_clk (kb_timer_device_t dev)
{
    switch (dev) {
    case KB_TIMER_TIM1:     
        __TIM1_CLK_ENABLE();
