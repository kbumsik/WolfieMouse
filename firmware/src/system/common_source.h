...
// library message printing
        ...
        #include "terminal.h"
        #define KB_DEBUG_MSG(msg, ...)	terminal_printf("KB_LIB:" KB_MSG_BASE ":%d:" msg, __LINE__ ,##__VA_ARGS__)
        #define KB_DEBUG_WARNING(msg, ...)	terminal_printf("KB_LIB:" KB_MSG_BASE ":%d:Warning:" msg, __LINE__ , ##__VA_ARGS__)
        #define KB_DEBUG_ERROR(msg, ...)	terminal_printf("KB_LIB:" KB_MSG_BASE ":%d:Error: " msg, __LINE__ , ##__VA_ARGS__)
...
// library status return setting
    ...
    #define KB_CONVERT_STATUS(hal_status) do{   \
        ...
        {   \
        case HAL_OK:    \
            hal_status = (status_t)KB_OK; \
            break;  \
        case HAL_ERROR: \
            hal_status = (status_t)KB_ERROR;  \
            break;  \
        case HAL_BUSY:  \
            hal_status = (status_t)KB_BUSY;   \
            break;  \
        case HAL_TIMEOUT:   \
            hal_status = (status_t)KB_TIMEOUT;    \
            break;  \
        default:    \
            hal_status = (status_t)KB_UNKNOWN;   \
            break;  \
        }   \
    }while(0)
#else
