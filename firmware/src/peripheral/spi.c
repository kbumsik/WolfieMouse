...
static uint32_t get_bus_freq_(spi_t spi);
static SPI_HandleTypeDef *get_handler (spi_t spi);
static void enable_spi_clk_ (spi_t spi);
...
inline int spi_send(spi_t spi, uint8_t* buf, uint16_t size)
{
    return spi_send_timeout(spi, buf, size, TIMEOUT_MAX);
}
...
{
    return spi_receive_timeout(spi, buf, size, TIMEOUT_MAX);
}
...
inline int spi_sendreceive(spi_t spi, uint8_t *tx_buf, uint8_t *rx_buf, uint16_t size)
{
    return spi_sendreceive_timeout(spi, tx_buf, rx_buf, size, TIMEOUT_MAX);
}
