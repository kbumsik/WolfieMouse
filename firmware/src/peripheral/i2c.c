...
static I2C_HandleTypeDef *get_handler (i2c_t i2c);
static void enable_i2c_clk_ (i2c_t i2c);
...
inline int i2c_send(i2c_t i2c, uint16_t address_target, uint8_t* buf, uint16_t size)
{
    return i2c_send_timeout(i2c, address_target, buf, size, TIMEOUT_MAX);
}
...
{
    return i2c_receive_timeout(i2c, address_target, buf, size, TIMEOUT_MAX);
}
