...
{
    gpio_set(HCMS_290X_SCK_PORT, HCMS_290X_SCK_PIN, (gpio_state_t)input);
}
static inline void _CE_set(int input)
{
    gpio_set(HCMS_290X_CE_PORT, HCMS_290X_CE_PIN, (gpio_state_t)input);
}
static inline void _RS_set(int input)
{
    gpio_set(HCMS_290X_RS_PORT, HCMS_290X_RS_PIN, (gpio_state_t)input);
}
static inline void _RESET_set(int input)
{
    gpio_set(HCMS_290X_RESET_PORT, HCMS_290X_RESET_PIN, (gpio_state_t)input);
}
...
static void _write_ctrl_reg(uint8_t data)
{
    ...
    // write
    spi_send(HCMS_290X_SPI, &data, 1);

    //end
    delay_us(10);
    _CE_set(1);   //latch on
    uint8_t dummy = 0x00;
    spi_send(HCMS_290X_SPI, &dummy, 1);
}
...
void hcms_290x_init(void)
{
    ...
    gpio_init(HCMS_290X_CE_PORT, HCMS_290X_CE_PIN, &gpio_setting);

    // RS pin
    gpio_setting = (gpio_init_t ){
        .Mode = GPIO_MODE_OUTPUT_PP,
        .Pull = GPIO_PULLUP,
        .Speed = GPIO_SPEED_FREQ_VERY_HIGH // 50MHz
    };
    gpio_init(HCMS_290X_RS_PORT, HCMS_290X_RS_PIN, &gpio_setting);
    ...
    gpio_init(HCMS_290X_RESET_PORT, HCMS_290X_RESET_PIN, &gpio_setting);

    // MOSI pin
    spi_mosi_pin(HCMS_290X_SPI, HCMS_290X_MOSI_PORT, HCMS_290X_MOSI_PIN, NOPULL);

    // SCK pin
    spi_sck_pin(HCMS_290X_SPI, HCMS_290X_SCK_PORT, HCMS_290X_SCK_PIN, NOPULL);
